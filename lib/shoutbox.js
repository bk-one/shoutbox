...
Shoutbox.prototype.updateStatusOnShoutbox = function( shoutboxId, shoutbox, updateData, status ) {
  this.db.collection('shoutboxes', function(err, collection) {
    ...
    slug = DataValidator.makeSlug(updateData.group + "-" + updateData.statusId);
    updateData.updatedAt = new Date().getTime();
    shoutbox.groups[updateData.group][updateData.statusId] = { status: updateData.status, updatedAt : updateData.updatedAt, message: updateData.message, slug: updateData.slug }
